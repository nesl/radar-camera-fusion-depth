...
def train(train_image_path,
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        for batch_data in train_dataloader:
            ...
            image, radar_point, ground_truth_depth = batch_data

            # Apply augmentations and data transforms
            [image], [ground_truth_depth], [radar_point] = train_transforms.transform(
                images_arr=[image],
            ...
            '''
            logits = radarnet_model.forward(image, radar_point, return_logits=True)
...
def forward(model, image, radar_points, device=torch.device('cuda')):
