...
import networks
from uformer_model import Uformer, Downsample, Upsample
import numpy as np

class FusionNetTransformerModel(object):
    '''
    Image radar fusion
    ...
    def __init__(self,
        ...
        self.input_channels = input_channels_image + input_channels_depth

        self.transformer = Uformer(
                            img_size=256, 
                            in_chans=1, 
                            dd_in=self.input_channels,
                            embed_dim=32, 
    ...
    def forward(self, image, input_depth, return_multiscale=False):
        ...
        concatenated_input = torch.concat([image, input_depth], dim=1)

        outputs = self.transformer(concatenated_input)
    ...
    def save_model(self, checkpoint_path, step, optimizer):
        ...
        checkpoint['tranformer_state_dict'] = self.transfomer.state_dict()
