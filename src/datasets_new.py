...
class RadarNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        radar_points[:,0] += self.pad_size_x

        b_boxes = []

        for radar_point in radar_points:
            x1 = point[0] - patch_size[1] // 2
            x2 = point[0] + patch_size[1] // 2
            y1 = 0
            y2 = shape[0]
            bounding_box = np.asarray([x1,y1,x2,y2])
            b_boxes.append(bounding_box)
        ...
        return image, radar_points, ground_truth, b_boxes
